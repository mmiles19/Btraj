...
void rcvOdometryCallbck(const nav_msgs::Odometry odom)
{
    ...
    // _start_vel(0) = cos(yaw);
    // _start_vel(1) = sin(yaw);
    // _start_vel(2) = 0;
...
//    pcl::fromROSMsg(pcl_tf, cloud);
    ...
    Quaterniond origin_local_rotation(1, 0.0, 0.0, 0.0);
    ...
    /* Fake ceiling for planning ground robot */
...
/*
    float xwall, ywall, zwall;
    pcl::PointXYZ wallpt;
    // left wall
    for (zwall = 0;zwall <= 2;zwall +=0.1)
    {
	for (xwall = -3.3;xwall <= 0;xwall +=0.1)
	{
	ywall = -3.5;
	wallpt.x = xwall;
	wallpt.y = ywall;
	wallpt.z = zwall;
	cloud.push_back(wallpt);
	}
    }
    ...
    for (zwall = 0;zwall <= 2;zwall +=0.1)
    {
	for (ywall = -3.5;ywall <= 3.3;ywall +=0.1)
	{
	xwall = -3.3;
	wallpt.x = xwall;
	wallpt.y = ywall;
...
*/
    ...
    if( _enable_replanning )
    {
      if( checkExecTraj() || errorOdomPos() )
      {
          trajPlanning();
          ROS_INFO("Replanning");
      }
...
bool errorOdomPos()
{
 ...
 //   float errorzlim = errorxlim;
 //   ROS_INFO("poscmd subs = %f, %f, %f", _cmd.position.x, _cmd.position.y, _cmd.position.z);
 //   ROS_INFO("odom subs = %f, %f, %f", _start_pt(0), _start_pt(1), _start_pt(2));
 //   ROS_INFO("diff = %f, %f, %f", abs(_start_pt(0)-_cmd.position.x),abs(_start_pt(1)-_cmd.position.y),abs(_start_pt(2)-_cmd.position.z));
		ROS_INFO("odom: %.2f, %.2f, %.2f",_odom.pose.pose.position.x,_odom.pose.pose.position.y,_odom.pose.pose.position.z);
		ROS_INFO("cmd: %.2f, %.2f, %.2f",_cmd.position.x,_cmd.position.y,_cmd.position.z);
		ROS_INFO("diff: %.2f, %.2f, %.2f",
			abs(_odom.pose.pose.position.x - _cmd.position.x),
			abs(_odom.pose.pose.position.y - _cmd.position.y),
			abs(_odom.pose.pose.position.z - _cmd.position.z));
    ...
    // if ( (abs(_start_pt(0) - _cmd.position.x) >=errorxlim) || (abs(_start_pt(1) - _cmd.position.y) >=errorylim))
		if ( (abs(_odom.pose.pose.position.x - _cmd.position.x) >=errorxlim) || (abs(_odom.pose.pose.position.y - _cmd.position.y) >=errorylim))
		{
    	ROS_INFO("Error large");
    	return true;
    }
...
void trajPlanning()
{
   ROS_INFO("%d, %d, %d",_has_target, _has_map, _has_odom);
...
int main(int argc, char** argv)
{
    ...
    // _vis_map_pub = nh.advertise<sensor_msgs::PointCloud2>("vis_map", 1);
    _poscmd_sub = nh.subscribe( "position_cmd",  1, rcvPosCmdCallBack);
    ...
    nh.param("enable_replanning", _enable_replanning, false);
...
/*
        try{
           listener.lookupTransform("/world", "/camera_depth_optical_frame", ros::Time(), transf);
        }
